...
/* ===== module: navigation.js ===== */
    /** Coordinates view routing, bottom navigation state, and per-view scroll restoration.
        History-API routing, push half (docs, #99 C7): every show function records its route
        with history.pushState({view, sub, ...}) so system Back works; the pop half is the
        `popstate` listener at the bottom of app-bootstrap.js, which maps the recorded state
        back onto these show functions. */
    ...
    /* Shows the exercise library view. */
    ...
    /* Shows the workout tab (start screen, never straight into the editor). */
    ...
    /* Shows the stats view (skeleton first, deferred hydration). */
    function showStats(push = true) {
      rememberScroll(); state.selected = null; state.activeView = 'stats'; hideAllViews();
      $('#statsView').classList.add('active'); setActiveNav('stats');
      /* #409 (user 2026-09-14): paint the skeleton synchronously so the
         first paint isn't blank, then hydrate on a later frame — the PR and
         aggregation scans are the slow part of this view. Repeat visits skip
         both the skeleton and the deferred frame: renderStats() paints
         synchronously over the existing content (no shimmer flash). */
      if(!statsHydrated){
        paintStatsSkeleton();
        const hydrateStats=()=>{try{renderStats();restoreScroll('stats');}catch(err){console.error(err);}};
        if(typeof requestAnimationFrame==='function')requestAnimationFrame(()=>requestAnimationFrame(hydrateStats));
        else hydrateStats();
      }else{
        renderStats();restoreScroll('stats');
      }
      if (push) history.pushState({view:'stats'}, '', '#stats');
    }
    /* Shows the settings view, remembering the return tab. */
    function showSettings(push = true) {
      ...
      $('#settingsView').classList.add('active'); setActiveNav('settings'); renderSettings(); applySettingsSectionsState(); restoreScroll('settings');
...
/** Bottom-tab taps always land at the top of the destination page and never
    ...
    /* One-level Back on the Workout tab: pops the topmost sub-screen only.
       ...
       toHome (tab tap) collapses everything to the start screen — a stashed
       share return is consumed, not restored. */
    function collapseWorkoutSubScreen(toHome){
      ...
      if(state.workoutEditorOpen&&workoutState.draft){
        state.workoutEditorOpen=false;
        /* #217: a share-started session backs out to the share landing, not
           the start screen. One-shot: the stash is consumed on restore so a
           later Back can't resurrect it. */
        if(state.shareReturn){
          if(toHome)state.shareReturn=null;
          else{state.sharePreview=state.shareReturn;state.shareReturn=null;}
        }
        renderWorkoutScreen();window.scrollTo({top:0});return true;
      }
    ...
    function collapseWorkoutToStart(){
      ...
      /* #217: tab tap is "go home" — consume a stashed share return instead
         of restoring the landing. */
      if(collapseWorkoutSubScreen(true))return;
    ...
    function showProgramHome(){
      ...
      state.savedProgramPreviewId=null; /* #455: the tab always lands on the program home, never a saved preview. */
    ...
    /* Serializes a return route into a history-registry key. */
    ...
    /* Routes Back from a completed workout to its recorded destination. */
