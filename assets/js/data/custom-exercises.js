...
/* ===== module: custom-exercises.js ===== */
    ...
    function customOptions() {
      return {
        ...
        tracking:['reps','time','distance']
      };
    }

    /* Renders one pill group for the custom-exercise form. */
    ...
    /* Renders the custom-exercise form's pill controls and wires their toggles. */
    ...
    /* Opens the custom-exercise dialog (prefilled when editing an existing one). */
    ...
    /* Closes the custom-exercise dialog. */
    function closeCustomDialog() {
      $('#customExerciseDialog').close();
    }

    /* Rebuilds the library filter UI after a custom-exercise change. */
    ...
    /* A8 (#99): soft-delete. The record keeps its id in the store with a
       deletedAt timestamp, so history, stats, and PRs keep resolving. It is
       filtered out of the library, picker, and filter UI. */
    ...
    /* Opens the delete-confirmation dialog for a custom exercise. */
    function requestDeleteCustomExercise(id) {
      ...
      showModalPinned($('#deleteCustomDialog'));
    }
    /* Wires the delete-confirmation dialog buttons. */
