...
/* #99 C13: the "ob" prefix = onboarding-era, kept for continuity (this file was
   extracted from the parked onboarding gate). obState/obEl/obDispatchImport/…
   are import-engine names now, not onboarding names. */
...
function obEnsureGate(){
  ...
  gate.setAttribute('aria-label','Welcome to Kohlrabi');
  gate.innerHTML=`
    <div class="ob-phone">
...
function openCsvImport(){
  ...
  obWireImport();
  obWireRestoreDialogs();
  window.scrollTo(0,0);
}
/* Exits the standalone import screen and re-renders the current view. */
...
/* Hides the onboarding-gate overlay. */
function obHideGate(){
...
const OB_CSV_DEFAULT_TAGS=['Warmup','Dropset','Full ROM','Slow and controlled','Cheat set','Paused','Assisted','Failure'];
let obCsvNameToId=null, obCsvIdSet=null, obCsvStemToId=null, obCsvAliasToId=null;
/* Lowercase alphanumeric-only form for header/name matching. */
function obCsvNorm(s){return (s||'').toLowerCase().replace(/[^a-z0-9]/g,'');}
/* Builds the lazily-cached name/id/stem/alias lookup maps for exercise matching. */
...
/* Display name for a catalog id (falls back to the raw id). */
...
/* Detects the CSV dialect (native, Hevy, MacroFactor) from the header row. */
...
const OB_HEVY_SET_TAGS={'warm up':'Warmup','warmup':'Warmup','failure':'Failure','drop set':'Dropset','dropset':'Dropset'};
const OB_HEVY_MONTHS={jan:1,feb:2,mar:3,apr:4,may:5,jun:6,jul:7,aug:8,sep:9,oct:10,nov:11,dec:12};
/* Parses a Hevy date string to ISO (YYYY-MM-DD). */
...
/* Parses CSV text into normalized rows, the detected format, and any errors. */
...
/* Short base-36 hash of a string for batch dedupe keys. */
function obCsvHash(s){let h=5381;for(let i=0;i<s.length;i++)h=((h<<5)+h+s.charCodeAt(i))>>>0;return h.toString(36);}
/* True when the string is a valid ISO calendar date. */
function obValidIsoDate(s){
  if(!/^\d{4}-\d{2}-\d{2}$/.test(s||''))return false;
  const d=new Date(s+'T12:00:00');
  return !isNaN(d.getTime())&&d.toISOString().slice(0,10)===s;
}
/* Canonicalizes a CSV tag, accepting pre-rename aliases. */
function obCanonicalCsvTag(raw){
  const t=String(raw||'').toLowerCase();
  for(const tag of OB_CSV_DEFAULT_TAGS)if(tag.toLowerCase()===t)return tag;
  /* #484: pre-#383 alias — "To failure" was renamed to "Failure"; accept the
     old name so exports from before the rename still import cleanly. */
  if(t==='to failure')return 'Failure';
  return null;
}
/* Parses a reps cell; ranges become a note instead of a number. */
...
/* Parses a numeric cell, returning null when empty or invalid. */
...
/* Name candidates for "Base (Equipment)" style CSV exercise names. */
...
/* Tokenizes an exercise name for fuzzy matching. */
...
/* Extracts the equipment/grip/position tokens from a token list. */
function obFzEquipSet(tokens){
  const s={};
  tokens.forEach(function(t){if(OB_FZ_EQUIP.indexOf(t)>=0)s[t]=1;});
  return s;
}
/* Drops ignorable implement/grip/position words, keeping movement words. */
...
/* True when a fuzzy candidate may auto-match (no equipment contradiction). */
...
/* Fuzzy-matches an exercise name against the catalog, best first. */
...
/* Picks the confident auto-match from fuzzy candidates (fewest extra movement words). */
...
/* Resolves a CSV exercise name to a catalog id (matched/blank/unmatched). */
function obResolveCsvExercise(name){
  obCsvBuildLookup();
  const raw=obCleanExerciseName(name);
  if(!raw)return {status:'blank'};
  let r=null;
  if(obCsvIdSet[raw])r={status:'matched',id:raw,via:'id'};
  else{
    const id=obCsvNameToId[obCsvNorm(raw)];
    if(id)r={status:'matched',id:id,via:'name'};
    else{
      const stemId=obCsvStemToId[obFzTokens(raw).join('')];
      if(stemId)r={status:'matched',id:stemId,via:'stem'};
      else{
        const aliasId=obCsvAliasToId[obCsvNorm(raw)];
        if(aliasId)r={status:'matched',id:aliasId,via:'alias'};
      }
    }
  }
  if(!r){
    ...
    for(const c of cands){
      ...
      if(aid){r={status:'matched',id:aid,via:'alias'};break;}
    }
  }
  if(!r){
    const fz=obCsvFuzzy(raw,5);
    const obx=obCsvObvious(raw,fz);
    if(obx)r={status:'matched',id:obx.id,via:'fuzzy'};
  }
  /* #539: a CSV naming a merged variant resolves to the canonical exercise —
     hidden variants never enter the imported workout. */
  if(r&&r.status==='matched'&&typeof canonicalExerciseId==='function')
    r.id=canonicalExerciseId(r.id);
  return r||{status:'unmatched'};
}
/* Groups normalized rows into workouts by date + workout name. */
...
/* Plans an import: batch id, format, workout groups, match statuses. */
...
/* Builds one set from a CSV row, tagging warmup/failure/dropsets. */
function obBuildCsvSet(row,tagState){
  ...
  if(toFailure){const tf=obCanonicalCsvTag('Failure');if(tf&&tags.indexOf(tf)<0)tags.push(tf);}
  ...
  /* #564: imported numerics go through the same canonical sanitizer as
     live logging — a CSV with fractional reps or absurd weights can't bypass
     the clamp. obParseCsvNumber already nulls non-numerics; the sanitizer
     adds the integer-reps and upper-bound rules. */
  return {set:{w:sanitizeSetValue('w',obParseCsvNumber(row.weight_lb)),r:sanitizeSetValue('r',pr.r),seconds:sanitizeSetValue('seconds',obParseCsvNumber(row.duration_s)),rpe:sanitizeSetValue('rpe',obParseCsvNumber(row.rpe)),tags:tags,complete:true},note:pr.note};
}
/* Reads the stored list of already-imported batch ids. */
...
/* Records a batch id as imported so re-imports are skipped. */
...
/* Renders the import screen ("Bring your training with you"). */
function obRenderImport(){
  return `
  <div class="ob-screen">
    <span class="ob-kicker">Import</span>
    <h1 class="ob-h1">Bring your training with you</h1>
    <p class="ob-sub">Upload a workout-history CSV, a MacroFactor spreadsheet, or an app backup to restore.</p>
    <div class="ob-upload" id="obUploadBox" role="button" tabindex="0" aria-label="Upload a workout file">
      ...
      <div class="ob-hint">History CSV, MacroFactor program (.xlsx), or app backup (.json)</div>
      <input type="file" id="obCsvFile" accept=".csv,text/csv,.xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,.json,application/json" hidden>
    </div>
    <button class="ob-btn-link" id="obCsvHelpExpander" type="button" aria-expanded="false">What can I upload?</button>
    <p class="ob-template"><a href="assets/workout-history-template.csv" download="workout-history-template.csv">Download the column template</a></p>
    <div class="ob-help" id="obCsvHelp" hidden>
      ...
      <p><strong>App backup:</strong> a <code>workout-app-backup-*.json</code> file from Settings → Export. Restoring <strong>replaces</strong> all current data with the backup — you confirm twice before anything changes, and anything that isn&rsquo;t an app backup is rejected.</p>
...
/* Wires the per-workout skip checkboxes in the import preview. */
...
/* Summary line for the import review (how many exercises need review). */
...
/* Human label for an import source (Hevy/MacroFactor/CSV). */
...
/* Renders the import preview for history imports. */
...
/* Wires the "match to existing exercise" dropdowns in the preview. */
...
/* Routes the reviewed import plan to the right writer: MacroFactor program sheets
   become saved-workout templates, everything else becomes logs. */
function obDispatchImport(){
  const plan=obState.csvPlan;
  /* #368: a validated app backup doesn't import — it restores (wholesale
     replace), behind a two-step destructive confirmation. */
  if(plan&&plan.kind==='backup-restore'){obStartBackupRestoreConfirm();return;}
  if(plan&&plan.kind==='templates')obExecuteMfTemplateImport();
  else obExecuteCsvImport();
}
/* ===== #368: restore from an app backup (.json) ===== */
/* Strict validation that a picked file is the app's own backup format
   (the JSON written by exportWorkoutData()/downloadWorkoutBackup()).
   PURE: parses and checks only — nothing is mutated before this passes.
   Returns {ok:true,data} or {ok:false,reason}. */
const OB_BACKUP_ARRAY_KEYS=['completed','templates','tags','exerciseTagPresets','archivedPrograms','savedPrograms','customExercises','favorites','restDays']; /* restDays is legacy-only (feature removed 2026-09-22): old backups still validate, but the key is ignored on restore — nothing reads it anymore. */
const OB_BACKUP_OBJECT_KEYS=['activeProgram','draft','savedBuilder','progressionSetup','savedFilter'];
const OB_BACKUP_STRING_KEYS=['dashboardPeriod','statsPeriod','logPeriod'];
/* #561: deep validation — the top-level shape check let malformed nested
   rows through (exercises:"x", sets:[null], w:"abc"), which poisoned state
   and crashed screens after restore. Nested containers must be the right
   kind and set/exercise leaves must be objects with sane numerics. PURE. */
function obBackupSetsOk(sets){
  if(!Array.isArray(sets))return false;
  for(const s of sets){
    if(!s||typeof s!=='object'||Array.isArray(s))return false;
    for(const k of ['w','r','seconds','distance']){
      const v=s[k];
      if(v!==undefined&&v!==null&&v!==''&&!(typeof v==='number'&&Number.isFinite(v)))return false;
    }
    const rpe=s.rpe;
    if(rpe!==undefined&&rpe!==null&&rpe!==''&&!(typeof rpe==='number'&&Number.isFinite(rpe)))return false;
  }
  return true;
}
/* True when the backup's exercises array has a sane shape. */
function obBackupExercisesOk(exercises){
  if(!Array.isArray(exercises))return false;
  for(const ex of exercises){
    if(!ex||typeof ex!=='object'||Array.isArray(ex))return false;
    if(ex.sets!==undefined&&!obBackupSetsOk(ex.sets))return false;
  }
  return true;
}
/* True when the backup's workouts array has a sane shape. */
function obBackupWorkoutsOk(arr){
  if(!Array.isArray(arr))return false;
  for(const w of arr){
    if(!w||typeof w!=='object'||Array.isArray(w))return false;
    if(w.exercises!==undefined&&!obBackupExercisesOk(w.exercises))return false;
  }
  return true;
}
/* True when the backup's program object has a sane shape. */
function obBackupProgramOk(p){
  if(!p||typeof p!=='object'||Array.isArray(p))return false;
  if(p.workouts!==undefined){
    if(!Array.isArray(p.workouts))return false;
    for(const w of p.workouts){
      if(!w||typeof w!=='object'||Array.isArray(w))return false;
      const t=w.template;
      if(t!==undefined&&(typeof t!=='object'||t===null||Array.isArray(t)))return false;
      if(t&&t.exercises!==undefined&&!obBackupExercisesOk(t.exercises))return false;
    }
  }
  return true;
}
/* Validates backup JSON, returning {ok} or {ok:false, reason}. */
function obValidateBackupJson(text){
  let data=null;
  try{data=JSON.parse(text);}catch(_){return {ok:false,reason:'That file isn\u2019t valid JSON.'};}
  const notOurs='That file doesn\u2019t look like an app backup.';
  if(!data||typeof data!=='object'||Array.isArray(data))return {ok:false,reason:notOurs};
  if(typeof data.version!=='number')return {ok:false,reason:notOurs};
  if(typeof SCHEMA_VERSION==='number'&&data.version>SCHEMA_VERSION)return {ok:false,reason:'That backup was made by a newer version of the app. Update the app first, then restore it.'};
  if(typeof data.savedAt!=='number')return {ok:false,reason:notOurs};
  const allowed=new Set(['version','savedAt'].concat(OB_BACKUP_ARRAY_KEYS,OB_BACKUP_OBJECT_KEYS,OB_BACKUP_STRING_KEYS));
  for(const k of Object.keys(data))if(!allowed.has(k))return {ok:false,reason:notOurs};
  for(const k of OB_BACKUP_ARRAY_KEYS)if(k in data&&!Array.isArray(data[k]))return {ok:false,reason:notOurs};
  for(const k of OB_BACKUP_OBJECT_KEYS){const v=data[k];if(k in data&&(v===undefined||(v!==null&&(typeof v!=='object'||Array.isArray(v)))))return {ok:false,reason:notOurs};}
  for(const k of OB_BACKUP_STRING_KEYS)if(k in data&&typeof data[k]!=='string')return {ok:false,reason:notOurs};
  /* #561: nested shape checks — a backup with the right top-level keys but
     malformed workouts/exercises/sets must be rejected, not restored. */
  if('completed' in data&&!obBackupWorkoutsOk(data.completed))return {ok:false,reason:notOurs};
  if('templates' in data&&!obBackupWorkoutsOk(data.templates))return {ok:false,reason:notOurs};
  if('draft' in data&&data.draft!==null){
    const d=data.draft;
    if(d.exercises!==undefined&&!obBackupExercisesOk(d.exercises))return {ok:false,reason:notOurs};
  }
  if('activeProgram' in data&&data.activeProgram!==null&&!obBackupProgramOk(data.activeProgram))return {ok:false,reason:notOurs};
  for(const k of ['archivedPrograms','savedPrograms']){
    if(k in data){
      if(!Array.isArray(data[k]))return {ok:false,reason:notOurs};
      for(const p of data[k])if(!obBackupProgramOk(p))return {ok:false,reason:notOurs};
    }
  }
  if('customExercises' in data){
    for(const ex of data.customExercises)if(!ex||typeof ex!=='object'||Array.isArray(ex))return {ok:false,reason:notOurs};
  }
  if('restDays' in data){
    /* Legacy key (feature removed 2026-09-22): still shape-checked so old
       backups validate, but ignored on restore. */
    for(const d of data.restDays)if(typeof d!=='string')return {ok:false,reason:notOurs};
  }
  return {ok:true,data:data};
}
/* Summarizes a validated backup (date, counts per section). */
function obBackupSummary(data){
  const n=a=>Array.isArray(a)?a.length:0;
  return {
    date:new Date(data.savedAt),
    completed:n(data.completed),
    templates:n(data.templates),
    programs:n(data.archivedPrograms)+n(data.savedPrograms)+(data.activeProgram?1:0),
    customExercises:n(data.customExercises)
  };
}
/* One-line "N completed workouts · N saved workouts · …" summary. */
function obBackupCountLine(s){
  const bit=(n,noun)=>n+' '+noun+(n===1?'':'s');
  return bit(s.completed,'completed workout')+' · '+bit(s.templates,'saved workout')+' · '+bit(s.programs,'program')+' · '+bit(s.customExercises,'custom exercise');
}
/* Step 0 — the validated backup's preview card. The shared Import button
   becomes "Restore backup" and opens the two-step confirmation. */
function obRenderBackupRestorePreview(data,fileName){
  const box=obEl('obImportPreview'),btn=obEl('obImportNowBtn'),note=obEl('obUploadNote');
  const s=obBackupSummary(data);
  const dateStr=s.date.toLocaleDateString(undefined,{year:'numeric',month:'short',day:'numeric'})+' '+s.date.toLocaleTimeString(undefined,{hour:'numeric',minute:'2-digit'});
  if(note)note.textContent='';
  box.innerHTML=
    '<div class="ob-imp-sum"><strong>App backup</strong> from '+escapeHtml(dateStr)+
    (fileName?' <span class="ob-muted">('+escapeHtml(fileName)+')</span>':'')+
    '<br>'+escapeHtml(obBackupCountLine(s))+'.</div>'+
    '<div class="ob-imp-err">Restoring <strong>replaces all</strong> current data with this backup. This can\u2019t be undone.</div>';
  obState.backupRestore={data:data};
  obState.csvPlan={kind:'backup-restore',batchId:'backup-'+data.savedAt};
  btn.hidden=false;
  btn.textContent='Restore backup';
}
/* Shows the invalid-backup error and clears the pending restore. */
function obBackupRestoreInvalid(reason){
  obState.backupRestore=null;obState.csvPlan=null;
  const box=obEl('obImportPreview'),btn=obEl('obImportNowBtn'),note=obEl('obUploadNote');
  if(box)box.innerHTML='';
  if(btn)btn.hidden=true;
  /* Nothing was mutated: validation runs before any write, so a rejected
     file always leaves the current data exactly as it was. */
  if(note)note.textContent=reason+' Nothing was changed.';
}
/* Step 1 — first destructive confirmation. Fills the dialog with the backup's
   date and what the replace will remove, then shows it. */
function obStartBackupRestoreConfirm(){
  const pending=obState.backupRestore;
  if(!pending||!pending.data){obBackupRestoreInvalid('Nothing to restore.');return;}
  const s=obBackupSummary(pending.data);
  const dateStr=s.date.toLocaleDateString(undefined,{year:'numeric',month:'short',day:'numeric'})+' '+s.date.toLocaleTimeString(undefined,{hour:'numeric',minute:'2-digit'});
  const desc=obEl('restoreBackupDesc');
  if(desc)desc.textContent='This deletes your current data and replaces it with the backup from '+dateStr+' ('+obBackupCountLine(s)+').';
  const dlg=obEl('restoreBackupDialog');
  if(dlg&&typeof showModalPinned==='function')showModalPinned(dlg);
  else if(dlg&&dlg.showModal)dlg.showModal();
}
/* Step 2 — final confirmation, then the wholesale replace:
   tombstone-everything-not-in-the-backup (local tombstones keep deletes
   consistent across tabs), write the validated blob, reload into the
   restored state. */
function obExecuteBackupRestore(){
  const pending=obState.backupRestore;
  const note=obEl('obUploadNote');
  if(!pending||!pending.data){if(note)note.textContent='Nothing to restore.';return;}
  const data=pending.data;
  /* Defense in depth: re-validate the exact object about to be written —
     the preview-time check already passed, but no write happens on trust. */
  if(!obValidateBackupJson(JSON.stringify(data)).ok){obBackupRestoreInvalid('That backup didn\u2019t validate.');return;}
  /* An older backup runs the same migration pipeline the boot path uses, so
     the written blob is current before it ever hits storage. */
  try{if(typeof SCHEMA_VERSION==='number'&&data.version<SCHEMA_VERSION&&typeof runBlobMigrations==='function')runBlobMigrations(data);}catch(_){}
  if(note)note.textContent='Restoring backup…';
  const writeAndReload=async()=>{
    /* Public fork: no cross-tab sync — a backup restore writes the blob
       straight to local storage and reloads into it. */
    if(typeof Storage!=='undefined'&&Storage&&typeof Storage.saveBlob==='function'){
      await Storage.saveBlob(PERSIST_KEY,JSON.stringify(data));
    }else{
      localStorage.setItem(PERSIST_KEY,JSON.stringify(data));
    }
    /* The migration backup must not resurrect pre-restore state. */
    try{localStorage.removeItem(LS_BACKUP_KEY);}catch(_){}
    /* The write landed — reload straight into the restored state. */
    location.reload();
  };
  try{
    const p=writeAndReload();
    if(p&&typeof p.catch==='function')p.catch(()=>{if(note)note.textContent='Couldn\u2019t save the backup. Nothing was changed.';});
  }catch(_){
    if(note)note.textContent='Couldn\u2019t restore that backup. Nothing was changed.';
  }
}
/* Wire the two restore dialogs once (they live in index.html next to the
   delete-all dialog; the validated backup waits in obState.backupRestore). */
let obRestoreDialogsWired=false;
/* Wires the backup-restore confirmation dialogs (once). */
function obWireRestoreDialogs(){
  if(obRestoreDialogsWired)return;obRestoreDialogsWired=true;
  const close=id=>{const d=obEl(id);if(d)d.close();};
  const show=id=>{const d=obEl(id);if(!d)return;if(typeof showModalPinned==='function')showModalPinned(d);else if(d.showModal)d.showModal();};
  if(obEl('restoreBackupKeep'))obEl('restoreBackupKeep').addEventListener('click',()=>close('restoreBackupDialog'));
  if(obEl('restoreBackupCancel'))obEl('restoreBackupCancel').addEventListener('click',()=>close('restoreBackupDialog'));
  if(obEl('restoreBackupContinue'))obEl('restoreBackupContinue').addEventListener('click',()=>{close('restoreBackupDialog');show('restoreBackupConfirmDialog');});
  if(obEl('restoreBackupBack'))obEl('restoreBackupBack').addEventListener('click',()=>close('restoreBackupConfirmDialog'));
  if(obEl('restoreBackupBackBtn'))obEl('restoreBackupBackBtn').addEventListener('click',()=>close('restoreBackupConfirmDialog'));
  if(obEl('restoreBackupConfirm'))obEl('restoreBackupConfirm').addEventListener('click',()=>{close('restoreBackupConfirmDialog');obExecuteBackupRestore();});
}
/* Wires the import screen's upload/preview/confirm events. */
function obWireImport(){
  ...
  csvFile.addEventListener('change',()=>{
    ...
    csvFile.value='';
    if(!file)return;
    /* #368: .json → app backup → validate, then the restore flow (wholesale
       replace behind a two-step confirmation). Rejected files change nothing. */
    if(/\.json$/i.test(file.name||'')){
      obEl('obUploadNote').textContent='Reading backup…';
      const reader=new FileReader();
      reader.onload=()=>{
        const check=obValidateBackupJson(reader.result);
        if(check.ok)obRenderBackupRestorePreview(check.data,file.name);
        else obBackupRestoreInvalid(check.reason);
      };
...
/* Reads a zip's central directory into name→entry records. */
...
/* Inflates raw-deflate bytes via DecompressionStream. */
...
/* Parses an xlsx shared-strings part into a string array. */
...
/* Reads an xlsx ArrayBuffer into named sheet grids. */
...
/* Maps a MacroFactor set-type label to tags + note. */
function obMfTemplateSetTag(type){
  ...
  if(t.indexOf('failure')>=0)return {tags:['Failure'],note:''};
  return {tags:[],note:'Set type "'+String(type).trim()+'" has no mapping — kept as plain sets'};
}
/* Parses a MacroFactor template sheet into workout blocks. */
...
/* Plan the template import: same exercise-matching as history, but the
   payload is saved workouts (targets), not logs. */
...
/* Executes the planned MacroFactor template import into saved workouts. */
...
const OB_MF_SET_TAGS={'standard set':'','warmup set':'Warmup','warm-up set':'Warmup','failure set':'Failure','drop set':'Dropset','drop':'Dropset'};
/* Maps MacroFactor history rows onto the normalized import row shape. */
