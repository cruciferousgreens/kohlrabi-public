/* ===== module: share-codec.js ===== */
    ...
    /* #99 C10: the UTF-8 smuggling idiom. btoa only handles Latin-1, so JSON
       containing non-ASCII text must be UTF-8-encoded first: encodeURIComponent
       percent-encodes the bytes, unescape() turns those %XX sequences back into
       single Latin-1 chars btoa can digest. The decode side mirrors it
       (atob → escape → decodeURIComponent). v1 links are plain JSON; v2 links
       use the byte functions below instead. */
    function shareEncodeV1(obj){
      return btoa(unescape(encodeURIComponent(JSON.stringify(obj))))
        .replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'');
    }
    /* Decodes a v1 (uncompressed base64) share string. */
    function shareDecodeV1(str){
      return JSON.parse(decodeURIComponent(escape(atob(str.replace(/-/g,'+').replace(/_/g,'/')))));
    }
    /* #99 C10: the chunk loop (CH=0x8000) is deliberate, not a stylistic quirk.
       String.fromCharCode.apply(null, bigArray) throws a call-stack/range
       overflow on large arrays — slicing into 32K-element chunks keeps it
       safe for any payload size. The -/_/trailing-= rewrites make the output
    ...
    /* #99 C10: deflate is the browser-native zlib compressor
       (CompressionStream), async because it streams — Blob → stream →
       pipeThrough → arrayBuffer. v2 links compress the slimmed JSON before
       base64url so a 3-exercise workout shrinks from ~2700 chars to ~330. */
    ...
    /* v2 compact schema (user 2026-09-12: "much shorter links" — v2 never
       ...
       set: positional [w,r,seconds,rpe,tags?,targetRpe?,distance?], trailing
       empties trimmed (targetRpe is trailing so pre-A7 links decode
       identically; distance is trailing so pre-#498 links decode identically)
       exercise: {i:id,k:tracking?,m:metrics?,o:note?,g:tags?,u:supersetId?,p:prog?,e:[sets]}
    ...
    /* #498: distance rides as the 7th positional, trailing so older links
       decode identically. */
    function slimShareSet(s){
      ...
      const a=[(s.w!==''&&s.w!=null)?s.w:null,(s.r!==''&&s.r!=null)?s.r:null,
        ...
        (s.distance!==''&&s.distance!=null)?s.distance:null];
      while(a.length&&a[a.length-1]==null)a.pop();
      return a;
    }
    /* Compacts a progression config into short keys for share payloads. */
    ...
    /* Compacts an exercise into short keys for share payloads. */
    function slimShareExercise(item){
      const o={i:item.exerciseId};
      if(item.tracking&&item.tracking!=='reps')o.k=item.tracking;
      /* #498: a distance metric profile travels so the recipient logs
         distance the same way (time is already covered by `k`). */
      if(Array.isArray(item.metrics)&&item.metrics.includes('distance'))o.m=[...item.metrics];
    ...
    function slimSharePayload(payload){
      ...
      if(payload.kind==='template'){
        ...
        /* #570: the v2 slimming dropped schedule — a shared program lost its
           cover's "N sessions per week" line on import. Carry it compactly
           (dropped when empty, like the other optional fields). */
        if(src.schedule)slim.p.d=src.schedule;
      }
      if(payload.customExercises&&payload.customExercises.length)slim.c=payload.customExercises;
      return slim;
    }
    /* Expands a slim share progression back to full keys. */
    ...
    function expandSharePayload(slim){
      ...
      if(out.kind==='template'){
        ...
        out.program={name:sp.n||slim.n,length:sp.l??4,startWeek:sp.s??1,
          focus:sp.f||'',schedule:sp.d||'',progression:sp.g||null,
          workouts:(sp.w||[]).map(w=>({name:w.n,
            template:{name:w.t||w.n,exercises:(w.e||[]).map(expandEx)}}))};
      }
      return out;
    }
    /* Encodes a payload: slim, JSON, deflate, then base64url. */
    ...
    /* #459 (user 2026-09-15): a shareable program payload from ANY
       program-shaped object — the active program or a saved (inactive)
       one. buildProgramShare keeps the active-program entry point. */
    function buildProgramShareFrom(p){
      if(!p)return null;
      /* Persona-4 finding 3 (agent 2026-09-16): the cover's "N sessions per
         week" line renders from `schedule`, so the share payload carries it —
         share-imported programs keep the line. Legacy links without it
         decode to '' (the cover branch stays hidden). */
      const program=JSON.parse(JSON.stringify({name:p.name,length:p.length,startWeek:p.startWeek,
        focus:p.focus||'',schedule:p.schedule||'',progression:p.progression||null,
    ...
    /* Builds a share payload for the active program. */
    function buildProgramShare(){
      return buildProgramShareFrom(workoutState.activeProgram);
    }
    /* ---- Import side ---- */
    ...
    /* Validator: a share set's fields are sane numbers/tags. */
    ...
    /* Validator: a share progression config uses known values. */
    ...
    /* Validator: a share exercise is well-formed. */
    function validShareExercise(item){
      ...
      /* #498: metrics is a small known set. */
      if(item.metrics!==undefined&&item.metrics!==null&&(!Array.isArray(item.metrics)||!item.metrics.length||!item.metrics.every(m=>m==='load'||m==='reps'||m==='time'||m==='distance')))return false;
    ...
    /* Validator: an exercise list within the size cap. */
    ...
    /* Validator: custom exercise stubs are well-formed. */
    ...
    /* Validator: a full share payload (template or program). */
    function validSharePayload(p){
      ...
      /* Persona-4 finding 3: schedule rides the payload — validate it like
         any other share string (A10: no unbounded/garbage strings in). */
      if(pr.schedule!==undefined&&pr.schedule!==null&&!shareStrOk(pr.schedule,SHARE_MAX_NAME))return false;
      return true;
    }
