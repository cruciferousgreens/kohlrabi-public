...
describe('share codec',()=>{
  ...
  it('v2 round-trips through shareDecodeAny',async()=>{
    ...
    assert.deepEqual(stripUids(ex0.sets[0]),
      {w:135,r:8,seconds:'',distance:'',rpe:7,targetRpe:'',tags:['Warmup'],complete:false});
    assert.deepEqual(stripUids(ex0.sets[1]),
      {w:185,r:5,seconds:'',distance:'',rpe:9,targetRpe:'',tags:[],complete:false});
  });
  ...
  it("#360: scheme:'off' survives the v2 slim/expand round-trip",()=>{
    const p=payloadFixture();
    p.template.exercises[0].progression.scheme='off';
    const back=expandSharePayload(slimSharePayload(p));
    assert.equal(back.template.exercises[0].progression.scheme,'off');
  });
  ...
  it("#570: program schedule survives the v2 slim/expand round-trip",async()=>{
    const prog={v:2,kind:'program',name:'PPL',program:{name:'PPL',length:6,
      startWeek:1,focus:'',schedule:'mon,wed,fri',progression:null,workouts:[{name:'Push',
        template:{name:'Push',exercises:[
          {uid:'x1',exerciseId:'bench-press',tracking:'reps',note:'',exerciseTags:[],
           supersetId:null,progression:null,
           sets:[{uid:'s1',w:'',r:'',seconds:'',rpe:'',targetRpe:'',tags:[],
             complete:false}]}]}}]},customExercises:[]};
    const back=await shareDecodeAny(await shareEncodeV2(prog));
    assert.equal(back.kind,'program');
    assert.equal(back.program.schedule,'mon,wed,fri');
  });
