...
describe('renderWorkoutTemplateList — #232 tappable "clearing the search"',()=>{
  ...
  it('tapping the link clears the search box and the filters',()=>{
    ...
    assert.deepEqual(state.savedFilter,{muscles:[],inProgram:false,shared:false},'filters cleared'); /* #231: shared reset too */
  });
  ...
  it('#271: unfiltered empty state uses the corrected wording',()=>{
    renderWorkoutTemplateList();
    // Persona 1 (2026-09-15): the empty state names both creation paths —
    // the + Add button below and Save as template on a finished log.
    assert.ok(host.innerHTML.includes('Save as template on a finished log'),
