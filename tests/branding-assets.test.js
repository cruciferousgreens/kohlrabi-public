...
const PROD_CARD='https://kohlrabi.us/share-card-home-1200.png';
...
describe('home share card (v1.013)',()=>{
  ...
  it('og/twitter tags use a relative card URL (works at domain root and under /kohlrabi-public/)',()=>{
    const html=fs.readFileSync(path.join(ROOT,'index.html'),'utf8');
    assert.match(html,/<meta property="og:image" content="share-card-home-1200.png" \/>/);
    assert.match(html,/<meta name="twitter:image" content="share-card-home-1200.png" \/>/);
    assert.ok(!html.includes('content="/share-card-home-1200.png"'),'root-absolute card URL remains');
    assert.ok(!html.includes('content="https://kohlrabi.us/share-card-home-1200.png"'),'prod-absolute card URL remains');
