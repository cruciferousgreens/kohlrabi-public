...
const statsJs=fs.readFileSync(path.join(ROOT,'assets/js/pages/dashboard-stats.js'),'utf8');
const savedJs=fs.readFileSync(path.join(ROOT,'assets/js/pages/saved-workouts.js'),'utf8');
const detailJs=fs.readFileSync(path.join(ROOT,'assets/js/pages/exercise-detail.js'),'utf8');
const navJs=fs.readFileSync(path.join(ROOT,'assets/js/core/navigation.js'),'utf8');
...
describe('#324 saved-workout inline × delete',()=>{
  ...
  it('the × opens the same confirmation as the swipe rail',()=>{
    ...
    assert.ok(savedJs.includes("showModalPinned($('#deleteTemplateDialog'))"),'confirmation dialog shown (scroll-pinned)');
  });
