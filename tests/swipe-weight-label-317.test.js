...
describe('#317 deleting a middle set renumbers the survivors',()=>{
  it('the renumber loop writes 1..N in DOM order',()=>{
    const start=editor.indexOf('function renumberWorkoutSetRows(');
    assert.ok(start!==-1,'renumberWorkoutSetRows exists');
    const fn=editor.slice(start,editor.indexOf('\n    function deleteWorkoutSet(',start));
    assert.ok(fn.includes('numBtn.textContent=n'),
...
describe('surgical card swaps re-wire swipe (user 2026-09-13)',()=>{
  it('swapExerciseCard re-attaches swipe listeners to the rebuilt rows',()=>{
    // Adding warm-ups (or flipping Reps/Seconds) rebuilds the exercise card
    // via swapExerciseCard instead of the full render. The full render ends
    // with attachSwipeDelete; the swap must too, or every row on the card
    // goes dead — the "frozen" sets the user reported.
    const start=editor.indexOf('function swapExerciseCard');
    const end=editor.indexOf('function wireLiveTracking',start);
    assert.ok(start!==-1&&end!==-1,'swapExerciseCard found');
    const body=editor.slice(start,end);
    assert.ok(body.includes('attachSwipeDelete(newCard)'),
      'swapExerciseCard re-attaches swipe to the new card');
  });
  it('swapExerciseCard re-wires the Exercise-options controls too',()=>{
    // Same hole, same fix: the swap rebuilds the %1RM inputs and the
    // reps-only toggle, so without a re-wire they go dead on the swapped
    // card until the next full render.
    const start=editor.indexOf('function swapExerciseCard');
    const end=editor.indexOf('function wireLiveTracking',start);
    const body=editor.slice(start,end);
    assert.ok(body.includes('wireOnermOptionInputs(newCard,findDraftExercise'),
      '%1RM inputs re-wired on the swapped card');
    assert.ok(body.includes('wireRepsOnlyToggle(newCard,findDraftExercise'),
      'reps-only toggle re-wired on the swapped card');
  });
});

describe('Warm-up pill alignment (user 2026-09-13, icon removed 2026-09-22)',()=>{
  it('the pill takes the auto margin and pins itself to the right edge',()=>{
    assert.ok(css.includes('.exercise-accordion.workout-exercise .exercise-accordion-head > .warmup-corner { margin-left: auto; }'),
      'pill pushes itself to the right edge instead of hugging the title');
    assert.ok(!css.includes('.exercise-accordion.workout-exercise .exercise-accordion-head > .exercise-info-button'),
      'no info-icon margin rule remains in the card header');
  });
});
