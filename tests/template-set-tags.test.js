...
const setTagsSrc=fs.readFileSync(path.join(ROOT,'assets/js/workout/set-tags.js'),'utf8');
const savedWorkoutsSrc=fs.readFileSync(path.join(ROOT,'assets/js/pages/saved-workouts.js'),'utf8');
...
describe('set-tags.js resolves builder targets',()=>{
  ...
  it('tag changes persist immediately; the builder re-render is deferred to dialog close (#365)',()=>{
    assert.match(setTagsSrc,
      /function afterTagChange\(\) \{\s*if \(workoutState\.tagTarget\?\.mode === 'builder'\) \{ schedulePersist\(\); pendingBuilderTagRender=true; \}/,
      'builder tag edits persist and defer the builder re-render');
  });
});
