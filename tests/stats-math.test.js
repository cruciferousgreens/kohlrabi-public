...
describe('workoutsForPeriod',()=>{
  ...
  it('today: only today',()=>{
    periodFixtures();
    const got=workoutsForPeriod('today');
    const todayIso=localIsoDate(new Date());
    /* On Mondays the 'm' fixture lands on the same date as 't' — pin the
       invariant (every result is from today) instead of an exact id list. */
    assert.ok(got.some(w=>w.id==='t'),"today's workout is included");
    assert.ok(got.every(w=>w.isoDate===todayIso),'nothing from another day leaks in');
  });
...
describe('#170 workedMuscles: bodyweight sets light the map, volume math untouched',()=>{
  ...
  it('muscleHeatmapMarkup keeps the empty-state when nothing was worked at all',()=>{
    ...
    assert.ok(html.includes('No weighted volume'),'true empty-state preserved');
  });
  it('user 2026-09-16: the anatomical map stays visible with no activity — blank map plus the empty note',()=>{
    for(const [volumes,bySets] of [[{},false],[{},true]]){
      const html=muscleHeatmapMarkup(volumes,new Set(),false,bySets);
      assert.ok(html.includes('class="anatomy-map"'),'map hidden with no activity');
      assert.ok(html.includes('heatmap-shell'),'map shell missing');
      assert.ok(!html.includes('heatmap-list'),'empty period must not render a muscle list');
    }
    const html=muscleHeatmapMarkup({},new Set(),false);
    assert.ok(html.includes('No weighted volume in this period.'),'empty note still shown beside the blank map');
  });
