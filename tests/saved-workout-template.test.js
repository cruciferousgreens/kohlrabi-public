...
function fakeTemplateDocument(){
  ...
  return {
    ...
    createElement(tag){
      const el={tag,type:'',id:'',className:'',textContent:'',innerHTML:'',
        _listeners:{},_attrs:{},_before:[],
        setAttribute(k,v){this._attrs[k]=v;},
        getAttribute(k){return this._attrs[k];},
        before(node){this._before.push(node);},
...
describe('saveCompletedAsTemplate (#240)',()=>{
  ...
  it('adds a Share button next to Start — the log is now a saved workout (#240 refinement)',()=>{
    const workout=mkWorkout();
    saveCompletedAsTemplate(workout,null);
    const share=fakeDoc.created.find(el=>el.id==='shareCompletedTemplateBtn');
    assert.ok(share,'a share button node is created');
    assert.equal(share.className,'icon-button');
    assert.equal(share.getAttribute('aria-label'),'Share Push Day');
    assert.equal(typeof share._listeners.click,'function','the Share action is wired');
    assert.ok(share.innerHTML.includes('<svg'),'carries the share icon');
  });
});
