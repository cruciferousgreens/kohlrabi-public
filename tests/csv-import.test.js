...
describe('Hevy mapping',()=>{
  ...
  it('golden end-to-end: hevy.csv → mapped rows',()=>{
    ...
    assert.equal(rows[2].tags,'Failure'); /* #383: renamed from 'To failure' */
...
describe('#383: imports canonicalize failure tags to the renamed "Failure" set tag',()=>{
  it('obCanonicalCsvTag maps "failure" to Failure, not the old name',()=>{
    assert.equal(obCanonicalCsvTag('failure'),'Failure');
    assert.equal(obCanonicalCsvTag('Failure'),'Failure');
    /* #484: the old "To failure" literal is kept as a backward-compatible
       import alias (the shipped template uses "Failure"). */
    assert.equal(obCanonicalCsvTag('To failure'),'Failure','legacy alias still imports');
  });
  it('obMfTemplateSetTag emits Failure for failure sets',()=>{
    assert.deepEqual(obMfTemplateSetTag('failure set'),{tags:['Failure'],note:''});
    assert.deepEqual(obMfTemplateSetTag('warmup set'),{tags:['Warmup'],note:''});
    assert.deepEqual(obMfTemplateSetTag('drop set'),{tags:['Dropset'],note:''});
  });
});
